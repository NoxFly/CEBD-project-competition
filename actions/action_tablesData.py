...
class AppTablesData(QDialog):
    ...
    def refreshTable(self, label, table, query):
        ...
        try:
            ...
            display.refreshLabel(
                label, "Impossible d'afficher les données de la table : " + repr(e))
        else:
    ...
    def refreshAllTables(self):
        ...
        self.refreshTable(self.ui.label_epreuves, self.ui.tableEpreuves,
                          "SELECT numEp, nomEp, formeEp, categorieEp, nbSportifsEp, dateEp FROM LesEpreuves")
        self.refreshTable(self.ui.label_inscriptions, self.ui.tableInscriptions,
                          "SELECT numIn, numEp FROM LesInscriptions")
        self.refreshTable(self.ui.label_resultats, self.ui.tableResultats,
                          "SELECT numEp, gold, silver, bronze FROM LesResultats")
        self.refreshTable(self.ui.label_sportifs, self.ui.tableSportifs,
                          "SELECT numSp, nomSp, prenomSp, pays, categorieSp, date(dateNaisSp) FROM LesSportifs_base")

        # afficher le contenu de la table LesDisciplines et ajouter l'attribut discipline dans l'affichage de la table LesEpreuves
        self.refreshTable(self.ui.label_disciplines,
                          self.ui.tableDisciplines, "SELECT nomDI FROM LesDisciplines")
